...
decodeBase32 string alphabet bitsPerChar = out <$> foldM writeOut (Out{buffer = 0, bits = 0, written = 0, out = []}) end
  where
    ...
    writeOut :: Out -> Char -> Either String Out
    writeOut acc c = do
        value <- maybeToEither "Not found char in base32 alphabet" $ Map.lookup c codes
...
encodeBase36 alphabet source = do
    ...
    let b58List = reverse $ Map.elems b58Context
            ...
            case Map.toList b58Context' !? idx of
                Just bit ->
                    let newCarry = carry' + (256 * snd bit)
                     in pure (newCarry `div` base, Map.insert (fst bit) (mod newCarry base) b58Context')
